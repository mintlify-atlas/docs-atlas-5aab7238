...
<Tabs>
  ...
  <Tab title="Standalone">
    ...
    - [Node.js LTS v12+](https://nodejs.org/en/) — check with `node --version`
...
When the backend is running (either via Docker or standalone), the full API is accessible at `http://localhost:5000` (standalone) or proxied through Traefik at `http://127.0.0.1:5000` (Docker). The Swagger UI at `/api/docs` documents every endpoint, shows request/response schemas, and supports authenticated test calls.
...
Authenticated `curl` requests use the same format:
